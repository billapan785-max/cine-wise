...
// --- NEW FEATURE: ADVANCED DATE ENGINE ---
const DateEngine = {
  getReleaseTag: (date: string) => {
    if (!date) return { label: 'TBA', color: 'text-zinc-500' };
    const diff = +new Date(date) - +new Date();
    const days = Math.ceil(diff / (1000 * 60 * 60 * 24));
    if (days > 0 && days <= 30) return { label: `IN ${days} DAYS`, color: 'text-yellow-500' };
    if (days > 30) return { label: 'COMING SOON', color: 'text-blue-500' };
    if (days <= 0 && days > -30) return { label: 'NEW RELEASE', color: 'text-green-500' };
  ...
  <div className="fixed inset-0 z-[700] bg-zinc-950 overflow-y-auto animate-in fade-in duration-700">
    <div className="max-w-4xl mx-auto px-6 py-32 space-y-24">
        <div className="space-y-6 text-center">
           ...
           <p className="text-red-600 font-black tracking-[0.5em] uppercase text-xs">2026 // Decentralized Access</p>
        </div>
        ...
        <div className="space-y-20">
          <section className="space-y-8">
             ...
             <div className="text-zinc-500 font-bold leading-loose text-sm md:text-lg space-y-6 italic">
                <p>CINEWISE operates on a zero-tracking principle. We do not store user data, IP addresses, or search history. All information is cleared upon session termination.</p>
                <p>All movie metadata is provided via the TMDB API. For high-precision image processing, we recommend using <span className="text-white underline">bgremoverai.online</span> as it offers free background removal with no login requirements.</p>
             </div>
          </section>
          ...
          <section className="space-y-8">
             ...
             <div className="text-zinc-500 font-bold leading-loose text-sm md:text-lg space-y-6 italic">
                <p>1. <span className="text-white">Content:</span> We do not host video files. All promotional material belongs to respective production studios.</p>
                <p>2. <span className="text-white">Liability:</span> The Scream Prank tool is intended for entertainment purposes only. Users are responsible for its ethical use.</p>
             </div>
          </section>
        </div>

        <div className="text-center pt-10">
          <button onClick={onClose} className="bg-white text-black font-black px-16 py-6 rounded-2xl hover:bg-red-600 hover:text-white transition-all text-xs uppercase tracking-widest">Close Archives</button>
        </div>
    </div>
  </div>
);
      ...
      <header className={`fixed top-0 w-full z-[100] transition-all duration-700 px-6 md:px-20 py-8 flex items-center justify-between gap-4 ${isScrolled || viewMode !== 'home' ? 'bg-zinc-950/90 border-b border-zinc-800/50 backdrop-blur-3xl py-5' : 'bg-transparent'}`}>
        ...
        <div className="flex items-center gap-4 flex-1 justify-end">
            <button onClick={() => setShowPrank(true)} className="flex-shrink-0 group relative overflow-hidden bg-red-600 px-6 md:px-10 py-3.5 rounded-xl text-[10px] font-black uppercase tracking-[0.3em] transition-all hover:scale-105 active:scale-95 shadow-2xl">
              <span className="relative z-10 flex items-center gap-3 text-white"> <span className="hidden md:inline">Scream Call</span></span>
            </button>
                  ...
                  <div key={m.id} onClick={() => setSelectedMovie(m)} className="group cursor-pointer">
                    <div className={`aspect-[2/3] overflow-hidden rounded-[2rem] md:rounded-[3rem] border-2 bg-zinc-900 relative shadow-2xl transition-all duration-700 ${hackerMode ? 'border-green-900' : 'border-zinc-900 group-hover:border-red-600'}`}>
                      ...
                      <div className="absolute top-4 right-4 z-20">
                         <span className={`bg-black/80 backdrop-blur-md px-3 py-1.5 rounded-full text-[8px] font-black tracking-widest ${dateTag.color}`}>
                            {dateTag.label}
                         </span>
                      </div>
